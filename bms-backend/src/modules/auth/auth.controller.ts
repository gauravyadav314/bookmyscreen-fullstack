...
export const sendOtp = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const ttl = 1000 * 60 * 5; // 5 mins for better dev UX;
        const expires = Date.now() + ttl;
        const data = `${normalizedEmail}.${otp}.${expires}`;
        const hashedOTP = OtpService.hashOTP(data);

        // 3. Send OTP to user's email;
        try {
            await OtpService.sendOTPtoEmail(normalizedEmail, otp);
        } catch (error) {
            console.log(error);
        ...
        res.json({
            ...
            email: normalizedEmail, msg: "OTP sent to email successfully ✅"
        })
...
export const verifyOTP = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const data = `${normalizedEmail}.${cleanOtp}.${expires}`;
